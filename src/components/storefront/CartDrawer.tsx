      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md h-full shadow-2xl flex flex-col border-l border-slate-200 dark:border-slate-800">
        ...
        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400 text-xs font-bold px-2 py-0.5 rounded-full">
              {cartItems.length}
            </span>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {cartItems.length === 0 ? (
            <div className="text-center py-16 space-y-3">
              ...
              <p className="text-xs text-slate-500">Explore products in the storefront catalog to add items.</p>
            </div>
          ) : (
            cartItems.map((item) => {
              ...
              return (
                <div
                  key={item.Cart_ID}
                  className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 flex gap-3 items-center"
                >
                  <img
                    src={product.Image}
                    alt={product.Name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      <div className="flex items-center border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 overflow-hidden text-xs">
                        <button
                          onClick={() => onUpdateQuantity(item.Cart_ID, item.Quantity - 1)}
                          className="px-2 py-0.5 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold"
                        >
        ...
        {cartItems.length > 0 && (
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30 space-y-4">
            ...
            <button
              ...
              className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all active:scale-98 text-sm"
            >
            ...
            <div className="flex items-center justify-center gap-1.5 text-[11px] text-slate-400">
              ...
              Encrypted Checkout & Order Guarantee
            </div>
          </div>
        )}
      </div>
